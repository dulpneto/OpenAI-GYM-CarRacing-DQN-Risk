    ...
    def step(self, dt):
        for w in self.wheels:
            ...
            for tile in w.tiles:
                ...
                if visited_tile_idx < tile.idx:
                    visited_tile = tile
                    visited_tile_idx = visited_tile.idx
                friction_limit = max(
                    friction_limit, FRICTION_LIMIT * tile.road_friction
                )
                self.grass = False
            # print('Grass', grass, 'Friction', friction_limit, 'Tiles', len(w.tiles), visited_tile_idx)
            # if visited_tile_idx > 0:
            #    print('IDX', visited_tile_idx, visited_tile.color, 'GOAL', visited_tile.goal, 'GRASS', visited_tile.grass)
            # Force
            ...
            # print('SKID', abs(force) > 2.0 * friction_limit)

            # Skid trace
            if abs(force) > 2.0 * friction_limit:
                if (
                    w.skid_particle
                    and w.skid_particle.grass == self.grass
                    ...
                    w.skid_particle = self._create_particle(
                        w.skid_start, w.position, self.grass
                    )
                    w.skid_start = None
            else:
                w.skid_start = None
