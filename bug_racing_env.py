...
class FrictionDetector(contactListener):
    ...
    def _contact(self, contact, begin):
        ...
        if GOAL_START_IDX < tile.idx < GOAL_END_IDX:
            tile.color[:] = GOAL_COLOR
            # it reached the goal
            # print('CONTATO FINAL')
            self.env.done = True
        elif GOAL_END_IDX <= tile.idx:
            tile.color = BG_COLOR
        else:
...
class CarRacing(gym.Env, EzPickle):
    ...
    def _create_track(self):
        ...
        if fixed_track:
            checkpoints = []
            checkpoints.append((0, 225.0, 0.0))
            checkpoints.append((2.8305948675672195, -62.0480238213336, 19.943973798406226))
            checkpoints.append((4.051833812270633, -54.40952253550535, -70.02558841668974))
            checkpoints.append((4.71238898038469, -4.133182947122317e-14, -225.0))
            self.start_alpha = -0.7853981633974483
        else:
            print(checkpoints)
            print(self.start_alpha)
        self.road = []
        ...
        for i in range(len(track)):
            ...
            t.road_friction = 1.0
            t.idx = i
            if GOAL_START_IDX < t.idx < GOAL_END_IDX:
                t.color = GOAL_COLOR
                t.goal = True
    ...
    def _render_road(self, zoom, translation, angle):
        ...
        self._draw_colored_polygon(
            self.surf, PLAYFIELD_BOUND, BG_COLOR, zoom, translation, angle, clip=False
        )
